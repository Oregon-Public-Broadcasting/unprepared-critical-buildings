	...
	var buildings_layer = L.layerGroup()
		...
		function style(feature) {
	        switch (feature.properties.Collapse_Potential) {
				case 'Very High (100%)': return {fillColor: "#e43611", color: "#e43611"};
	            case 'High (>10%)': return {fillColor: "#f25e0d", color: "#f25e0d"};
	            case 'Moderate (>1%)': return {fillColor: "#FFF79A", color: "#FFF79A"};
	            case 'Low (<1%)':   return {fillColor: "#a2cc61", color: "#a2cc61"};
	            default: return {fillColor: "#666", color: "#666"}
	       		}
	    	}
	    ...
	    var tsunami_geojson = L.geoJson(tsunami, {
	     style: function () {
			...
			var with_distances = _.filter(buildings, function(x, y){
				...
				$("#box-info").empty();
				$("#box-info").append("At least " + collapse_count + " critical buildings nearby have been rated a high risk of collapse.")
		...
		if(selection.length > 0){
			...
			// var box = '<div id="box" class="row"><div id="box-info"></div></div></div>'
					...
					if(is_city == true){
						...
						$('#buildings-list').append(selectedbuildingsView.render().el);
						$("#box-info").empty();
						$("#box-info").prepend("At least " + "<b>" + collapse_count  + "</b>" + " critical buildings nearby have been rated a high risk of collapse.")
						$("#buildings-container").show();
					...
					/**** IF THEY DON/T PICK A CITY ****/
						...
						$.getJSON('https://maps.googleapis.com/maps/api/geocode/json?address=' + selection + '&key=AIzaSyBSZGyycuPZO0BBfJqO-RBFeKM7_icZUnk', function(data) {
							...
							$('#buildings-list').append(selectedbuildingsView.render().el);
							$("#box-info").empty();
							$("#box-info").append("At least " + collapse_count + " critical buildings nearby rated high risk of collapse.")
							$("#buildings-container").show();
